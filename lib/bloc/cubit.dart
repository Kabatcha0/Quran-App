...
// عدد الايات فى كل سورة
  ...
  void binding(int index) {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      log("message");
      jumpToAyah(index: index);
    });
  }

  void jumpToAyah({required int index}) {
    if (fClicked) {
      itemScrollController
          .scrollTo(index: index, duration: const Duration(seconds: 2))
          .then((value) {
        log("======================");
        // fClicked = !fClicked;
        emit(QuranJumpSuccesslState());
      }).catchError((e) {
        log("==============================");
        log(e.toString());
        emit(QuranJumpErrorState());
      });
    }
  }
  ...
  List json = [];
  void fromJson() {
    rootBundle.loadString("assets/hafs_smart_v8.json").then((value) {
